      ...
      >
        {s.icon === "wallet" && (
          ...
          >
            ...
            <path d="M2 10h20" />
            <circle cx="17" cy="15" r="1" fill={s.iconColor} />
          </svg>
        )}
        {s.icon === "spend" && (
          <svg
...
const SpendingChart = ({ budgetData }) => {
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm p-6 md:p-8">
      <h3 className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] mb-1">
        Spending Breakdown
      </h3>
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span
                    style={{ fontSize: "9px", fontWeight: 900, color: color }}
                  >
...
const BudgetModal = ({ budgets, onSave, onClose }) => {
  ...
  const removeCategory = (id) =>
    setTempBudgets(tempBudgets.filter((b) => (b._id || b.id) !== id));
  ...
  const hasDuplicate = tempBudgets.some((b, _, arr) => {
    ...
    return (
      name && arr.filter((x) => x.name.trim().toLowerCase() === name).length > 1
    );
  });

  const isValid =
    tempBudgets.every((b) => b.name.trim() && b.limit > 0) && !hasDuplicate;
        ...
        <div className="p-6 md:p-8 space-y-4 overflow-y-auto bg-slate-50/30">
          ...
          {tempBudgets.length === 0 && (
            <div className="text-center py-16 text-slate-300 font-bold">
              No budgets yet — click Quick Add or Add New above
            </div>
          )}
          {tempBudgets.map((cat) => {
                ...
                <div className="flex flex-col md:flex-row gap-4 md:items-start">
                  <div className="flex-[2] flex flex-col">
                    ...
                    <input
                      value={cat.name}
                      onChange={(e) =>
                        updateCategory(id, "name", e.target.value)
                      }
                  ...
                  <div className="flex-1 flex flex-col">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 md:text-right">
                      Budget Limit
                    </label>
                    <input
                      type="text"
    ...
    >
      ...
      >
        <div>
          ...
          <span
            className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider mt-1 inline-block ${b.badge}`}
          >
            {b.label}
          </span>
          {b.createdAt && (
      ...
      >
        <span
          style={{
            ...
            fontWeight: 900,
            color:
              b.remaining < 0 ? "#ef4444" : hovered ? "#64748b" : "#94a3b8",
            transition: "color 0.2s ease",
          }}
        >
          {b.remaining < 0
    ...
    return budgets.map((cat) => {
      ...
      const spent = transactions
        .filter(
          (t) =>
            t.type === "expense" &&
            t.category?.toLowerCase().trim() === name?.toLowerCase().trim(),
        )
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-emerald-100 px-4 lg:px-10 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button
              ...
              className="lg:hidden p-2 text-gray-600 flex items-center justify-center"
            >
              <svg
                width="20"
                height="20"
            ...
            <div>
              ...
              <p className="text-[10px] text-emerald-600 font-black uppercase tracking-[0.15em]">
                Monthly Limits & Spending
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 lg:p-8 max-w-[1400px] mx-auto space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              {
                label: "Remaining",
                value: formatCurrency(totalAllocated - totalSpent),
                icon: "check",
                iconColor: "#10b981",
                accent: "#10b981",
              },
            ...
            <div className="lg:col-span-7 space-y-3">
              ...
              {budgetData.length === 0 ? (
                <div className="bg-white rounded-[2rem] border border-dashed border-slate-200 p-16 text-center">
                  ...
                  <div className="text-[16px] font-black text-slate-300 mb-2">
                    No budgets set yet
                  </div>
                  <div className="text-[12px] text-slate-300 mb-6">
              ...
              {budgetData.length > 0 ? (
                ...
                <div className="bg-white rounded-[2rem] border border-dashed border-slate-200 p-12 text-center">
                  ...
                  <div className="text-[12px] font-black text-slate-300">
                    Chart will appear once you have budgets and transactions
                  </div>
                </div>
              )}
            </div>
