...
function doExport(tab, filtered, label, budgets, taxpayerType, regime = "new") {
  ...
  if (tab === "income") {
    ...
    const rows = Object.entries(byM)
      .sort((a, b) => a[0].localeCompare(b[0]))
      .map(([m, d]) => ({
        Month: m,
        Income: inr(d.i),
        Expense: inr(d.e),
        Net_Savings: inr(d.i - d.e),
        Savings_Rate:
    ...
    dl(
      ...
      ["Month", "Income", "Expense", "Net_Savings", "Savings_Rate"],
    ...
    dl(
      [
        ...
        { Field: "Estimated Tax", Value: inr(estTax) },
        {
          ...
          Value: totInc > 0 ? ((estTax / totInc) * 100).toFixed(2) + "%" : "0%",
        },
    ...
    const rows = (budgets || []).map((b) => {
      ...
      return {
        ...
        Allocated: inr(alloc),
        Spent: inr(spent),
        Remaining: inr(alloc - spent),
        Status: spent > alloc ? "OVER BUDGET" : "OK",
      };
    });
    dl(
      rows.length
      ...
      <div className="flex flex-col gap-2 flex-1 min-w-0 w-full sm:w-auto">
        {paths.map((p, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            />
            <span className="text-[11px] font-bold text-gray-500 flex-1 min-w-0 break-words">
              {p.label}
            </span>
...
function BarChartSVG({ slices, colors }) {
  ...
  const bw = Math.max(
    18,
    Math.min(52, Math.floor((W - pl - pr) / slices.length) - 12),
  );
    ...
    <div className="bg-white rounded-2xl border border-gray-50 shadow-sm overflow-hidden">
      {(title || sub) && (
        <div className="px-6 pt-5 pb-2">
          ...
          {sub && (
            <p className="text-[11px] font-bold text-gray-500 mt-0.5">{sub}</p>
          )}
        </div>
      )}
      {children}
...
function TxnList({ txns, colorClass, sign }) {
  if (!txns.length)
    return (
      <p className="text-center py-8 text-[11px] font-black text-gray-300 uppercase tracking-widest">
        No transactions
      </p>
    );
  return (
    <div className="divide-y divide-gray-50">
      {txns.map((t, i) => (
        <div
          key={i}
          className="flex justify-between items-center px-6 py-3.5 hover:bg-gray-50/50 transition-colors"
        >
          <div>
          ...
          <p className={`text-sm font-black tracking-tighter ${colorClass}`}>
            {sign}
...
const Reports = ({
  ...
  const fileLabel = useMemo(
    () => makePeriodLabel(mode, year, month, day),
    [mode, year, month, day],
  );
  const badge = useMemo(
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              ...
              },
              {
                ...
                color: net >= 0 ? "text-gray-900" : "text-rose-500",
                bg: "bg-white",
              ...
              <div
                key={c.label}
                className={`${c.bg} rounded-2xl p-4 border ${c.border} shadow-sm`}
              >
                <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">
                  {c.label}
                </p>
                <p
                  className={`text-xl font-black ${c.color} tracking-tighter leading-none`}
                >
                  {c.value}
                </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card title="Income by Category — Donut">
                  <div className="px-6 pb-5">
                    <DonutChart slices={incSlices} />
                  </div>
                </Card>
                <Card title="Income by Category — Bar">
                  <div className="px-6 pb-5">
                    <BarChartSVG slices={incSlices} />
                  </div>
                </Card>
              </div>
            ...
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  {
                    label: "Net Savings",
                    val: formatCurrency(net),
                    col: net >= 0 ? "text-gray-900" : "text-rose-500",
                  },
                ].map((c) => (
                  <div
                    key={c.label}
                  ...
                  >
                    ...
                    <p
                      className={`text-2xl font-black ${c.col} tracking-tighter leading-none`}
                    >
                      {c.val}
                    </p>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card title="Income vs Expense — Donut">
                  <div className="px-6 pb-5">
                    <DonutChart
                      slices={[
                        { label: "Income", value: totInc, color: "#10b981" },
                        { label: "Expense", value: totExp, color: "#fb7185" },
                      ]}
                    />
                  </div>
                </Card>
              ...
              <Card title="Month-by-Month Breakdown">
                <div className="divide-y divide-gray-50 px-6">
                  {monthlyData.length === 0 && (
                    <p className="text-center py-6 text-[11px] font-black text-gray-300 uppercase tracking-widest">
                      No data
                    </p>
                  )}
                    ...
                    return (
                      <div key={idx} className="py-4">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <p className="text-sm font-black text-gray-900">
                              {new Date(d.label + "-01").toLocaleDateString(
                                "en-IN",
                                { month: "long", year: "numeric" },
                              )}
                            </p>
                            <p
                              className={`text-[10px] font-bold mt-0.5 ${n >= 0 ? "text-emerald-600" : "text-rose-500"}`}
                            >
                              Net: {n >= 0 ? "+" : ""}
                              {formatCurrency(n)} · Savings Rate:{" "}
                              {d.income > 0
                                ? ((n / d.income) * 100).toFixed(1)
                                : 0}
                              %
                            </p>
                          </div>
                        ...
                        <p className="text-[9px] text-gray-400 font-bold mt-1">
                          {ratio.toFixed(0)}% spent of income
                        </p>
                      </div>
                    );
                  })}
            ...
            <>
              {budgetData.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-[13px] font-black text-gray-700 mb-1">
                    No Budgets Found
                  </p>
                  <p className="text-[11px] text-gray-400 font-semibold">
                    Create budgets first to see budget report here.
                  </p>
                </div>
              ) : (
                <>
                    ...
                    {(() => {
                      ...
                      return [
                        ...
                        },
                        {
                          ...
                          col:
                            overCount > 0 ? "text-red-600" : "text-emerald-600",
                          bg: "bg-white",
                          border: "border-gray-100",
                        },
                      ].map((c) => (
                        <div
                          key={c.label}
                          className={`${c.bg} rounded-2xl p-5 border ${c.border} shadow-sm`}
                        >
                          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-2">
                            {c.label}
                          </p>
                          <p
                            className={`text-xl font-black ${c.col} tracking-tighter`}
                          >
                            {c.val}
                          </p>
                        </div>
                  ...
                  <Card title="Allocated vs Spent — Bar Chart">
                    <div className="px-6 pb-5">
                      <BarChartSVG
                        slices={budgetData.map((b) => ({
                          label: b.name,
                          value: b.spent,
                        }))}
                        colors={budgetData.map((b) =>
                          b.over ? "#ef4444" : "#10b981",
                        )}
                      />
                      <p className="text-[9px] text-gray-400 font-bold mt-2 text-center">
                        🔴 Red = Over budget · 🟢 Green = Within budget
                      </p>
                    </div>
                  </Card>
                        ...
                        <div key={i} className="px-6 py-4">
                          <div className="flex justify-between items-start mb-2">
                            <div className="flex items-center gap-2">
                              <span
                                className={`w-2 h-2 rounded-full flex-shrink-0 ${b.over ? "bg-red-400" : "bg-emerald-400"}`}
                              />
                              <span className="text-sm font-black text-gray-900">
                                {b.name}
                              </span>
                              {b.over && (
                                <span className="text-[8px] font-black bg-red-100 text-red-600 px-2 py-0.5 rounded-full uppercase tracking-wide">
                                  Over Budget
                                </span>
                              )}
                            </div>
                            <div className="text-right">
                              <p
                                className={`text-sm font-black tracking-tighter ${b.over ? "text-red-500" : "text-gray-900"}`}
                              >
                                {formatCurrency(b.spent)}{" "}
                                <span className="text-[10px] font-bold text-gray-400">
                                  / {formatCurrency(b.alloc)}
                                </span>
                              </p>
                              <p
                                className={`text-[10px] font-bold mt-0.5 ${b.rem >= 0 ? "text-emerald-600" : "text-red-500"}`}
                              >
                                {b.rem >= 0
                                  ? `${formatCurrency(b.rem)} left`
                                  : `${formatCurrency(-b.rem)} over`}
                              </p>
                            </div>
                          </div>
                          <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all ${b.over ? "bg-red-400" : "bg-emerald-400"}`}
                              style={{ width: `${b.pct}%` }}
                            />
                          </div>
                          <p className="text-[9px] text-gray-400 font-bold mt-1">
            ...
            <>
                  ...
                  <div className="px-6 pb-5">
                    <DonutChart
                      slices={[
                        {
                          label: "In-Hand",
                          value: Math.max(0, totInc - estTax),
                        },
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {quarterData.map((q, i) => (
                  <div
                    key={i}
                    className={`${QC[i].bg} rounded-2xl p-5 border ${QC[i].border}`}
                  >
                    <div className="flex justify-between items-center mb-2">
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest ${QC[i].col}`}
                      >
                        {q.label}
                      </span>
                      <span className={`text-[9px] font-black ${QC[i].col}`}>
                        {q.pct}%
                      </span>
                    </div>
                    <p
                      className={`text-xl font-black tracking-tighter ${QC[i].col} mb-1`}
                    >
                ...
                <div className="px-6 pb-6 space-y-1 mt-2">
                  {[
                    ...
                    },
                    {
                      ...
                      val:
                        totInc > 0
                          ? `${((estTax / totInc) * 100).toFixed(1)}%`
                          : "0%",
                      col: "text-blue-600",
                    },
                    {
                      label: "In-Hand After Tax",
                      val: formatCurrency(inHand),
                      col: "text-gray-900",
                    },
                  ].map((r, i) => (
                    <div
                      key={i}
                      className={`flex justify-between items-center px-4 py-3 rounded-xl ${i % 2 === 0 ? "bg-gray-50/60" : "bg-white"}`}
                    >
                      <span className="text-xs font-bold text-gray-500">
                        {r.label}
                      </span>
                      <span
                        className={`text-sm font-black tracking-tighter ${r.col}`}
                      >
                        {r.val}
                      </span>
                    </div>
