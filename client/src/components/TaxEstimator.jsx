...
const StatCard = ({ label, value, color, bg }) => (
  <div
    className="rounded-[11px] border border-slate-100 p-[11px_13px]"
    style={{ background: bg || "#f8fafc" }}
  >
    <div className="text-[9px] font-bold text-slate-400 uppercase tracking-[.12em] mb-[5px]">
      {label}
    </div>
    <div
      className="text-[15px] font-extrabold truncate tracking-[-0.02em]"
...
const TaxEstimator = ({ isDashboard = true }) => {
    ...
    return (
      <div className="bg-white rounded-[16px] border border-slate-100 shadow-sm p-5 flex flex-col items-center gap-3 text-center">
        ...
        <div className="text-[12px] font-bold text-slate-700">
          No Tax Calculation Saved
        </div>
        <div className="text-[10px] text-slate-400 font-semibold">
          Go to Tax Estimator, fill your details and save.
        </div>
        <button
  ...
  const paidPct =
    tax.totalTax > 0
      ? Math.min(
          100,
          Math.round(((tax.tdsAlreadyPaid || 0) / tax.totalTax) * 100),
        )
      : 100;
  const savedDate = tax.savedAt
    ? new Date(tax.savedAt).toLocaleDateString("en-IN", {
        day: "numeric",
        month: "short",
        year: "numeric",
      })
  ...
  return (
    <div className="bg-white rounded-[16px] border border-slate-100 shadow-sm flex flex-col gap-3 overflow-hidden">
      ...
      {tax.totalTax > 10000 && (
        <div
          className="mx-4 rounded-[12px] p-[11px_13px]"
        ...
        >
          <div className="flex justify-between items-start mb-2">
            <div>
              ...
              <div
                ...
                style={{ color: rDark, opacity: 0.75 }}
              >
                {nextQ.full} · Pay ₹
                {Math.round(nextQ.instalment).toLocaleString("en-IN")} this
                instalment
              </div>
            </div>
            <span
              className="text-[9px] font-extrabold text-white px-2 py-0.5 rounded-full"
              style={{ background: rc }}
            >
          ...
          <div
            ...
            style={{ color: rDark, opacity: 0.65 }}
          >
      ...
      <div className="flex items-center gap-2 px-4">
        ...
        <span className="text-[9px] font-bold text-slate-400 shrink-0">
          Effective Rate:{" "}
          <strong className="text-slate-600">{tax.effectiveRate}%</strong>
        </span>
        <div className="flex-1 h-[1px] bg-slate-100" />
      </div>
