    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {[1, 2, 3].map((n) => (
          <div
            key={n}
            className="bg-white p-6 rounded-[2.2rem] border border-gray-100 shadow-sm animate-pulse"
          >
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
      {cards.map((card, index) => (
        ...
        >
          ...
          <div
            className={`absolute top-0 left-0 right-0 h-1 ${card.accent} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
          ></div>

          <div className="flex items-center gap-4 md:gap-5 relative z-10">
            {/* Icon Container */}
            <div
              className={`${card.bg} w-14 h-14 md:w-16 md:h-16 rounded-[1.3rem] flex items-center justify-center text-2xl md:text-3xl shadow-inner group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 ease-out`}
            >
              {card.emoji}
            </div>
            ...
            <div className="flex flex-col">
              ...
              <h3
                className={`text-xl md:text-2xl font-black ${card.color} tracking-tighter leading-none`}
              >
