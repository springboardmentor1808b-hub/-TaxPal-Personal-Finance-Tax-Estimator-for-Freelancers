...
const BudgetProgress = ({
  ...
  const addCategory = () =>
    setTempBudgets([
      { id: `temp-${Date.now()}`, name: "", limit: 0 },
      ...tempBudgets,
    ]);
  const removeCategory = (id) =>
    setTempBudgets(tempBudgets.filter((cat) => (cat._id || cat.id) !== id));
  ...
  const handleConfirmSave = async () => {
    if (tempBudgets.some((b) => !b.name.trim() || !b.limit || b.limit <= 0))
      return;
    try {
        ...
        .filter(
          (t) =>
            ...
            t.category?.toLowerCase().trim() ===
              (cat.name || cat.category)?.toLowerCase().trim(),
        )
  ...
  if (loading)
    return (
      <div className="animate-pulse p-8 bg-white rounded-[2.5rem]">
        Loading...
      </div>
    );
  ...
  return (
    <>
      ...
      <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col min-h-[400px] lg:h-[520px] antialiased">
        <div className="mb-10">
          <h4 className="text-2xl font-[1000] text-slate-900 tracking-tight leading-tight">
            Budget Tracker
          </h4>
          <p className="text-[11px] text-emerald-600 font-black uppercase tracking-[0.15em] mt-1">
            Real-time Analytics
          </p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-2 scrollbar-hide">
          {budgetData.length === 0 ? (
            ...
            budgetData.map((b) => (
              <div
                key={b.id}
                className="group p-4 rounded-3xl transition-all duration-300 hover:bg-slate-50 border-l-4 border-transparent hover:border-emerald-500"
              >
                <div className="flex justify-between items-end mb-3">
                  <div className="space-y-1">
                    <span className="text-[14px] font-[1000] text-slate-800 uppercase tracking-wide group-hover:text-black transition-colors">
                      {b.name}
                    </span>
                    <p
                      className={`text-[10px] font-black tracking-wider ${b.remaining < 0 ? "text-rose-600 animate-pulse" : "text-slate-400"}`}
                    >
                      {b.remaining < 0
                        ? `⚠️ OVER: ${formatCurrency(Math.abs(b.remaining))}`
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-tighter">
                ...
                <div className="h-2.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full transition-all duration-1000 ease-out rounded-full ${b.statusColor}`}
        ...
        <button
          ...
          className="mt-8 w-full py-5 bg-slate-900 text-white rounded-[2rem] text-[11px] font-[1000] uppercase tracking-[0.2em] shadow-lg hover:bg-emerald-600 active:scale-[0.98] transition-all duration-300"
        >
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4 z-[9999]">
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="p-6 md:p-8 border-b flex justify-between items-center bg-white sticky top-0 z-10">
                <div>
                  <h3 className="text-xl md:text-2xl font-[1000] text-slate-900">
                    Budget Settings
                  </h3>
              ...
              <div className="p-6 md:p-8 space-y-6 overflow-y-auto scrollbar-hide bg-slate-50/30">
                {tempBudgets.map((cat) => {
                  ...
                  return (
                    <div
                      key={cat._id || cat.id}
                      className="relative bg-white p-5 md:p-6 rounded-[2rem] shadow-sm border border-slate-100 transition-all group-within:shadow-md"
                    >
                      <div className="flex flex-col md:flex-row gap-5 md:items-start">
                        ...
                        <div className="flex-[2] flex flex-col">
                          ...
                          />
                          {isNameInvalid && (
                            <span className="text-[9px] font-bold text-rose-500 uppercase mt-1.5 ml-1 animate-pulse">
                              Required name *
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex-1 flex flex-col">
                          <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1 md:text-right">
                            Budget Limit
                          </label>
                          <input
                          ...
                          />
                          {isLimitInvalid && (
                            <span className="text-[9px] font-bold text-rose-500 uppercase mt-1.5 ml-1 md:text-right md:mr-1 animate-pulse">
                              Invalid limit *
                            </span>
                          )}
                        </div>
                        ...
                        <button
                          onClick={() => removeCategory(cat._id || cat.id)}
                          className="md:hidden absolute top-5 right-6 text-rose-400 font-black text-[10px] uppercase"
                        >
                          Remove
                        </button>
                      </div>
              ...
              <div className="p-6 md:p-8 bg-white border-t sticky bottom-0">
                <button
                  ...
                  disabled={tempBudgets.some(
                    (b) => !b.name.trim() || !b.limit || b.limit <= 0,
                  )}
