...
const TransactionModal = ({
  ...
  const getOptions = () => {
    const budgetNames = userBudgets.map((b) =>
      typeof b === "object" ? b.name || b.category : b,
    );
    const defaults =
  ...
  const inputStyle =
    "w-full bg-gray-50 border border-gray-100 rounded-2xl px-4 sm:px-5 py-3.5 sm:py-4 outline-none focus:border-emerald-500 focus:bg-white transition-all font-medium text-sm";
    ...
    <div className="fixed inset-0 z-[110] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-gray-900/60 backdrop-blur-sm">
      <div className="bg-white w-full sm:max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col max-h-[92vh] sm:max-h-[90vh] animate-slideUp">
        ...
        <div className="px-6 sm:px-8 py-5 sm:py-6 flex justify-between items-center border-b border-gray-50 shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-50 text-gray-400 hover:bg-rose-50 hover:text-rose-500 transition-all text-xl"
          >
            &times;
          </button>
        </div>
        ...
        <form
          ...
          className="px-6 sm:px-8 py-5 sm:py-6 space-y-4 sm:space-y-5 overflow-y-auto"
        >
          ...
          {!initialData ? (
            ...
            <div className="flex items-center justify-center pb-1">
              <span
                className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.1em] shadow-sm border ${
                  formData.type === "income"
